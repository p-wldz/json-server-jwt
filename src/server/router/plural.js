...
module.exports = (db, name, opts) => {
  ...
  function create(req, res, next) {
    ...
    if (payload = jwt.isLogged(req))
    {
      if (opts._isFake) {
        const id = db
          ...
          .createId()
          .value()
        resource = { ...req.body, id, userId: payload.id }
      } else {
        let data = { ...req.body, userId: payload.id }
        resource = db
          .get(name)
          .insert(data)
          .value()
    ...
    {
      res.status(401)
      res.locals.data = {status: 400, message: 'You are not logged'}
    }
